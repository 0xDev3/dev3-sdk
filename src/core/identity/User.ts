...
export class User {
  ...
  public async addToAddressBook(
    ...
    return MainApi.instance().createAddressBookEntry(request, this.jwt);
  }

  public async getFromAddressBook(alias: string): Promise<AddressBookEntry> {
    return MainApi.instance().fetchAddressBookEntryByAlias({ alias }, this.jwt);
  }

  public async getAllFromAddressBook(): Promise<AddressBookEntries> {
    return MainApi.instance().fetchAddressBookEntries(this.wallet, this.jwt);
  }

  public async updateAddressBook(
    request: UpdateAddressBookEntryRequest
  ): Promise<AddressBookEntry> {
    return MainApi.instance().updateAddressBookEntry(request, this.jwt);
  }
}
